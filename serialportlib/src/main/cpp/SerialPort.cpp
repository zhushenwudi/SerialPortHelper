...
int SerialPort::setSpeed(int fd, int speed) {
    ...
    if (tcgetattr(fd, &cfg)) {
        LOGE("[%s] tcgetattr invocation method failed!", path);
        close(fd);
        return FALSE;
    }
    ...
    if (tcsetattr(fd, TCSANOW, &cfg)) {
        LOGE("[%s] tcsetattr invocation method failed!", path);
        close(fd);
        return FALSE;
    }
    return TRUE;
...
int SerialPort::setParity(int fd, int databits, int stopbits, char parity) {
    ...
    if (tcgetattr(fd, &options) != 0) {
        LOGE("[%s] The method tcgetattr exception!", path);
        return FALSE;
    }
    ...
    switch (databits)                                           /* Set data bits */
    {
        ...
        case 8:
            ...
            break;
        default:
            LOGE("[%s] Unsupported data size!", path);
            return FALSE;
    }
    switch (parity) {
        case 'n':
        ...
        case 's':  /*as no parity*/
            ...
            break;
        default:
            LOGE("[%s] Unsupported parity!", path);
            return FALSE;
    }
    /* 设置停止位*/
    switch (stopbits) {
        ...
        case 2:
            ...
            break;
        default:
            LOGE("[%s] Unsupported stop bits!", path);
            return FALSE;
    }
    ...
    if (tcsetattr(fd, TCSANOW, &options) != 0) {
        LOGE("[%s] The method tcsetattr exception!", path);
        return FALSE;
    }
    return TRUE;
}
...
int SerialPort::openSerialPort(SerialPortConfig config) {
    ...
    fd = open(path, O_RDWR);
    if (fd < 0) {
        LOGE("[%s] Error to read port file!", path);
        return FALSE;
    }

    if (!setSpeed(fd, config.baudrate)) {
        LOGE("[%s] Set Speed Error!", path);
        return FALSE;
    }
    if (!setParity(fd, config.databits, config.stopbits, config.parity)) {
        LOGE("[%s] Set Parity Error!", path);
        return FALSE;
    }
    LOGD("[%s] Open device Success!", path);
    return TRUE;
}
